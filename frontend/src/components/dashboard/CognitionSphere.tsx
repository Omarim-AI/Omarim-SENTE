...
const CognitionSphere: React.FC<CognitionSphereProps> = ({ position }) => {
  ...
  useEffect(() => {
    ...
    const fogGeometry = new THREE.SphereGeometry(7, 32, 32);
    const fogMaterial = new THREE.MeshStandardMaterial({ color: 'blue', transparent: true, opacity: 0.05, side: THREE.DoubleSide });
    fogRef.current = new THREE.Mesh(fogGeometry, fogMaterial);
  }, []);
  ...
  useFrame(({ clock }) => {
    ...
    nodesRef.current.forEach((node, i) => {
      ...
      const positions = (ribbonsRef.current[i].geometry.attributes.position as THREE.BufferAttribute).array as Float32Array;
      positions[0] = 0;
      positions[1] = 0;
      positions[2] = 0;
      positions[3] = node.x;
  ...
  return (
    <group position={position}>
      ...
      {fogRef.current && <primitive object={fogRef.current} />}
    </group>
  );
}

...
function getHaloColor(confidence: number): THREE.Color {
...
interface CentralCognitionSphereProps {
  aiConfidence?: number;
}

const CentralCognitionSphere: React.FC<CentralCognitionSphereProps> = ({ aiConfidence = 0.8 }) => {
  ...
  useFrame((state, delta) => {
    ...
    if (halo.current) {
        (halo.current.material as THREE.MeshBasicMaterial).opacity = 0.3 + pulse * 0.3 * pulseIntensity;
    }
  });
